...
import { updateFile } from "@/controllers/fileController";
import { Request } from "express";
import { EModelEndpoint } from "@/types";
import { IMongoFile } from "@/models/file";
/**
...
async function uploadLocalImage({
  ...
  const userPath = path.join(imageOutput, (req as any).user?.id ?? "");
 ...
 */
async function prepareImagesLocal(req: Request, file: IMongoFile) {
  ...
  const promises = [];
  promises.push(updateFile({ ...file, filepath }));
 ...
 */
async function processLocalAvatar({
  buffer,
  userId,
  manual,
  agentId,
