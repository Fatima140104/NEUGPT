...
async function resizeImageBuffer(
  inputBuffer: Buffer,
  resolution: "low" | "high" | { percentage?: number; px?: number },
  endpoint: EModelEndpoint
) {
  const maxLowRes = 512;
  const maxShortSideHighRes = 768;
  const maxLongSideHighRes =
    endpoint === EModelEndpoint.anthropic ? 1568 : 2000;
  ...
  if (customPercent != null || customPx != null) {
    ...
    if (customPercent != null) {
      ...
      newWidth = Math.min(metadata.width ?? 0, customPx ?? 0);
      newHeight = Math.min(metadata.height ?? 0, customPx ?? 0);
    }
